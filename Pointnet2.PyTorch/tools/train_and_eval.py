...
parser.add_argument("--net", type=str, default='pointnet2_ssgseg')
...
def train_one_epoch(model, train_loader, optimizer, epoch, lr_scheduler, total_it, log_f):
    model.train()
    ...
    for it, batch in enumerate(train_loader):
        ...
        print("pts_input.size()", pts_input.size())
...
def eval_one_epoch(model, eval_loader, epoch, log_f=None):
    model.train()
...
def train_and_eval(model, train_loader, eval_loader, ckpt_dir, log_f):
    model.cuda()
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        total_it = train_one_epoch(model, train_loader, optimizer, epoch, lr_scheduler, total_it, log_f)
...
if __name__ == '__main__':
    ...
    eval_set = KittiDataset(root_dir='/workspace/alvin/', mode='EVAL', split='val')
    ...
    if args.mode == 'train':
        train_set = KittiDataset(root_dir='/workspace/alvin/', mode='TRAIN', split='train')
        ...
        # train and eval
        train_and_eval(model, train_loader, eval_loader, ckpt_dir, log_f)
        log_f.close()
    elif args.mode == 'eval':
